...
var EXEC_URL = 'https://script.google.com/macros/s/AKfycbyZ_ArwdqpVL9JSfsQKl-Hv-qt_rdwKNFia1ZfBXV12COacMXlJDDz95JiPtU2TwLQf/exec';
...
function renderTechInfoPage() {
  ...
  if (info.marketing) {
    ...
    html += '<div class="techinfo-title" style="margin-top:22px;">Маркетинговые каналы</div>';
    html += '<div class="techinfo-explainer">' +
      '<b>Вес</b> — насколько сильно канал в принципе может повлиять на привлекательность заведения ' +
      'по сравнению с брендом, качеством и остальными каналами: выше вес — выше потолок влияния. ' +
      '<b>Альфа</b> — масштаб эффекта внутри самого канала: при бюджете, равном опорному, «сырой» эффект ' +
      'канала (до применения веса) примерно равен альфе — выше альфа, тем сильнее тот же бюджет работает ' +
